...
public class DiffRowGeneratorTest {
		...
		public void testNormalize_List() {
				...
				assertEquals(
								Collections.singletonList("    test"), generator.normalizeLines(Collections.singletonList("\ttest")));
		}
		...
		public void testGeneratorWithWordWrap() {
				...
				DiffRowGenerator generator = DiffRowGenerator.create().columnWidth(5).build();
				List<DiffRow> rows = generator.generateDiffRows(split(first), split(second));
				print(rows);
		...
		public void testGeneratorWithMerge() {
				...
				assertEquals(
								...
								rows.get(0).toString());
				assertEquals(
								"[CHANGE,<span class=\"editOldInline\"> </span>,]", rows.get(1).toString());
				assertEquals("[EQUAL,other,other]", rows.get(2).toString());
		}
		...
		public void testGeneratorWithMerge3() {
				...
				assertEquals(
								...
								rows.get(1).toString());
				assertEquals(
								"[DELETE,<span class=\"editOldInline\"> </span>,]", rows.get(2).toString());
		...
		public void testGeneratorWithMergeByWord5() {
				DiffRowGenerator generator = DiffRowGenerator.create()
								...
								.build();
				List<DiffRow> rows =
								generator.generateDiffRows(Arrays.asList("Test feature"), Arrays.asList("ester feature best"));
				print(rows);
		...
		public void testSplitString() {
				List<String> list =
								DiffRowGenerator.splitStringPreserveDelimiter("test,test2", DiffRowGenerator.SPLIT_BY_WORD_PATTERN);
		...
		public void testGeneratorExample1() {
				...
				List<DiffRow> rows = generator.generateDiffRows(
								Arrays.asList("This is a test senctence."), Arrays.asList("This is a test for diffutils."));
		...
		public void testGeneratorIssue14() {
				DiffRowGenerator generator = DiffRowGenerator.create()
								...
								.build();
				List<DiffRow> rows = generator.generateDiffRows(
								Arrays.asList("J. G. Feldstein, Chair"), Arrays.asList("T. P. Pastor, Chair"));
		...
		public void testGeneratorIssue22() {
				...
				List<DiffRow> rows = generator.generateDiffRows(Arrays.asList(aa.split("\n")), Arrays.asList(bb.split("\n")));
		...
		public void testGenerationIssue44reportLinesUnchangedProblem() {
				DiffRowGenerator generator = DiffRowGenerator.create()
								...
								.build();
				List<DiffRow> rows =
								generator.generateDiffRows(Arrays.asList("<dt>To do</dt>"), Arrays.asList("<dt>Done</dt>"));
		...
		public void testIgnoreWhitespaceIssue66() {
				...
				// CHECKSTYLE:OFF
				List<DiffRow> rows =
								generator.generateDiffRows(Arrays.asList("This\tis\ta\ttest."), Arrays.asList("This is a test"));
				// CHECKSTYLE:ON
		...
		public void testProblemTooManyDiffRowsIssue65() {
				...
				List<DiffRow> diffRows = generator.generateDiffRows(
								...
								Arrays.asList(
												"Ich möchte nicht mehr mit dir sprechen. Leite mich weiter.",
		...
		public void testLinefeedInStandardTagsWithLineWidthIssue81() {
				List<String> original =
								Arrays.asList(("American bobtail jaguar. American bobtail bombay but turkish angora and tomcat.\n"
		...
		public void testIssue86WrongInlineDiff() throws IOException {
				String original = Files.lines(Paths.get("target/test-classes/com/github/difflib/text/issue_86_original.txt"))
								.collect(joining("\n"));
				String revised = Files.lines(Paths.get("target/test-classes/com/github/difflib/text/issue_86_revised.txt"))
								.collect(joining("\n"));
				...
				DiffRowGenerator generator = DiffRowGenerator.create()
								...
								.build();
				List<DiffRow> rows =
								generator.generateDiffRows(Arrays.asList(original.split("\n")), Arrays.asList(revised.split("\n")));
		...
		public void testCorrectChangeIssue114() throws IOException {
				...
				assertThat(rows)
								.extracting(item -> item.getTag().name())
								.containsExactly("CHANGE", "DELETE", "EQUAL", "CHANGE", "EQUAL");
		}
		...
		public void testIssue119WrongContextLength() throws IOException {
				String original = Files.lines(Paths.get("target/test-classes/com/github/difflib/text/issue_119_original.txt"))
								.collect(joining("\n"));
				String revised = Files.lines(Paths.get("target/test-classes/com/github/difflib/text/issue_119_revised.txt"))
								.collect(joining("\n"));
		...
		public void testIssue129SkipWhitespaceChanges() throws IOException {
				String original = Files.lines(Paths.get("target/test-classes/com/github/difflib/text/issue129_1.txt"))
								.collect(joining("\n"));
				String revised = Files.lines(Paths.get("target/test-classes/com/github/difflib/text/issue129_2.txt"))
								.collect(joining("\n"));
		...
		public void testGeneratorWithWhitespaceDeltaMerge() {
				final DiffRowGenerator generator = DiffRowGenerator.create()
								...
								.inlineDeltaMerger(DiffRowGenerator.WHITESPACE_EQUALITIES_MERGER)
								.build();

				assertInlineDiffResult(generator, "No diff", "No diff", "No diff");
				assertInlineDiffResult(
								generator,
								" x whitespace before diff",
								" y whitespace before diff",
								" ~x~**y** whitespace before diff");
				assertInlineDiffResult(
								generator, "Whitespace after diff x ", "Whitespace after diff y ", "Whitespace after diff ~x~**y** ");
		...
		public void testGeneratorWithMergingDeltasForShortEqualities() {
				final Function<InlineDeltaMergeInfo, List<AbstractDelta<String>>> shortEqualitiesMerger =
								deltaMergeInfo -> DeltaMergeUtils.mergeInlineDeltas(
												deltaMergeInfo,
												equalities ->
																equalities.stream().mapToInt(String::length).sum() < 6);
				...
				final DiffRowGenerator generator = DiffRowGenerator.create()
								...
								.oldTag(f -> "~")
								.newTag(f -> "**")
								.inlineDeltaMerger(shortEqualitiesMerger)
								.build();
