		...
		for (int i = 0; i < patch.getDeltas().size(); i++) {
		    ...
		    if (orig.getSize() == rev.getSize()) {
		    	...
		    	for (int j = 0; j < rev.getSize(); j++) {
		    		diffRows.add(new DiffRow(Tag.CHANGE, orig.getLines().size() > j ? 
		    				(String)orig.getLines().get(j) : "", (String)rev.getLines().get(j)));
		    	}
		    }
		    endPos = orig.last() + 1;
		}
			...
			for (Delta inlineDelta: inlineDeltas) {
				...
				if (inlineDelta.getClass().equals(DeleteDelta.class)) {
					origList = wrapInTag(origList, inlineOrig.getPosition(),
							inlineOrig.getPosition() + inlineOrig.getSize() + 1, 
							this.InlineOldTag, this.InlineOldCssClass);
				} else if (inlineDelta.getClass().equals(InsertDelta.class)) {
					revList = wrapInTag(revList, inlineRev.getPosition(),
							  inlineRev.getPosition() + inlineRev.getSize() + 1, 
							  this.InlineNewTag, this.InlineNewCssClass);
